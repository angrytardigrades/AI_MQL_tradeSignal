...
//--- if the handle is not created
   ...
   int Report = FileOpen("report_final.csv",FILE_WRITE|FILE_CSV|FILE_ANSI,",",CP_ACP);
   FileWrite(Report,"datetime","open","high","low","close","tvol","spr","ccpi","ma","findex","rvi","rsi","signal");

   int signal [50000]; //-- 0 means no action , 1 =buy , 2= sell
   signal [0]=0;
   ...
   for(i=2; i<(bars-1); i++)
     {
      signal[i]=0;
      if(bar[i].low < bar[i-1].low && bar[i].close > ma[i])
        {
         if(bar[i-1].high>bar[i].high)
           {
            if(bar[i-1].high-bar[i-1].open > 0.00200)
              {
               signal [i]=1;
              }
           }
      ...
      if(bar[i].high > bar[i-1].high && bar[i].close < ma[i])
        {
         if(bar[i-1].low<bar[i].low)
           {
            if(bar[i-1].open-bar[i-1].low > 0.00200)
              {
               signal [i] =2;
              }
           }
   ...
   for(i=1; i<(bars-1); i++)
     {
      FileWrite(Report,date_time=iTime(_Symbol,PERIOD_CURRENT,i),bar[i].open,bar[i].high,bar[i].low,bar[i].close,bar[i].tick_volume,bar[i].spread,cci[i],ma[i],force_index[i],rvi[i],rsi[i],signal[i]);
     }
